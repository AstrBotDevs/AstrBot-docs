...
在管理面板的 `对话` 页面，你可以直接与 AstrBot 进行对话。

### 人格切换与编辑

在对话页面的顶部或侧边栏（取决于 UI 版本），你可以点击人格选择器来切换当前对话使用的人格。

- **切换人格**：点击列表中的人格即可切换。
- **新建人格**：点击选择器底部的 `新建人格` 按钮。
- **编辑人格**：在人格选择器列表中，点击人格名称旁的 **编辑图标（铅笔）**，即可直接修改该人格的配置（如系统提示词、名称等），无需跳转到专门的人格管理页面。
